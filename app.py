...
# ==========================================
# [최종 수정됨] 함수: 프롬프트 생성 (정보: 예전 버전 복구 / 역사: 색감+키워드)
# ==========================================
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info"):
    ...
    # ---------------------------------------------------------
    # [모드 1] 밝은 정보/이슈 (사용자 요청: 예전 버전 원복)
    # ---------------------------------------------------------
    if genre_mode == "info":
        # 사용자가 만족했던 그 프롬프트 지침 그대로 적용
        full_instruction = f"""
    [역할]
...
# ---------------------------------------------------------
    # [모드 2] 역사/다큐 (수정됨: 2D 스타일 강제 + 3D 금지 조항 추가)
    # ---------------------------------------------------------
    else: # genre_mode == "history"
        full_instruction = f"""
    [역할]
    당신은 역사적 사건을 진지한 화풍으로 담아내는 '2D 다큐멘터리 애니메이션 감독'이자 '역사 정밀 일러스트레이터'입니다.

    [전체 영상 주제]
    "{video_title}"

    [그림 스타일 가이드 - 유저 지정 (최우선 준수)]
    {style_instruction}
    
    [필수 연출 지침]
    1. **[매우 중요] 매체(Medium):** 무조건 **평면적인 '2D 일러스트레이션'** 또는 **'셀 애니메이션'** 스타일로 표현하십시오. **절대로 3D 그래픽, 모델링, 또는 실사 사진 같은 느낌이 나서는 안 됩니다.**
    2. **조명(Lighting):** 2D 작화 내에서 입체감과 깊이를 주는 **'시네마틱 조명(Cinematic Lighting)'**을 사용하십시오. 명암의 대비를 활용하되, 그림체는 평면적이어야 합니다.
    3. **색감(Colors):** **색감을 풍부하게 살리십시오.** 현대적인 쨍함이 아니라, 그 시대의 명화나 필름처럼 **깊이 있고 진한 색조(Rich & Deep Tones)**를 사용하여 시대적 분위기를 연출하십시오.
    4. **분위기(Mood):** 가볍지 않고 진중하며, 몰입감 있는 분위기를 만드십시오. (단, 실사 같은 현장감이 아닌 **잘 그려진 회화 작품**을 보는 듯한 느낌)
    5. **디테일(Detail):** 인물의 복식, 건축 양식, 소품 등의 역사적 고증에 신경 써서 사실적인 디테일을 2D로 표현하십시오.
    6. **텍스트(Text):** 대본의 핵심이 되는 **한글 키워드 1~2개**를 장면에 자연스럽게 녹여내십시오. (예: 오래된 현판, 깃발, 펼쳐진 문서, 비석 등에 쓰인 글씨처럼 투박하게 연출)
    7. **구성(Composition):** 시청자가 상황을 한눈에 이해할 수 있도록 핵심 피사체를 화면 중앙에 배치하십시오. 분활화면(Split screen)은 절대 금지입니다.
    ...
    - **분량:** 최소 5문장 이상으로 상세하게 묘사.
    - **포함 요소:**
        - **캐릭터 행동:** 대본의 상황을 연기하는 캐릭터의 구체적인 동작 (2D 스틱맨 스타일 유지).
        - **배경:** 상황을 설명하는 소품이나 장소 (시대적 배경을 2D로 묘사).
        - **시각적 은유/텍스트:** 내용에 맞는 시각적 요소 및 자연스러운 한글 텍스트 배치.
