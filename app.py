...
# ---------------------------------------------------------
    # [모드 2] 역사/다큐 (최종: 세계사/한국사 통합 + 2D/감정/배경/검열회피)
    # ---------------------------------------------------------
    ...
    2. **[매우 중요] 텍스트 현지화(Localization):** 배경이 서양, 중국, 일본 등 어디든 상관없이, 화면 속 글씨(간판, 지도, 깃발 등)는 **무조건 '한글'로 표기**하십시오.
        - **금지:** 알파벳(영어/독어 등), 한자, 일본어 사용을 절대 금지합니다.
        - **예시:** 'New York' -> **'뉴욕'**, 'Berlin' -> **'베를린'**, '大日本' -> **'일본'**, 'Qing' -> **'청나라'**
    ...
    - **분량:** 최소 5문장 이상으로 상세하게 묘사.
    - **포함 요소:**
        - **텍스트 지시:** (중요) 이미지에 들어갈 텍스트를 반드시 **'한글'**로 명시하십시오. (예: "지도 위에는 '소련'이라는 한글이 적혀 있다.")
