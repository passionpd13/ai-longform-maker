...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info"):
    ...
    # ---------------------------------------------------------
    # [모드 2] 역사/다큐 (색감 풍부 + 시네마틱 + 한글 키워드)
    # ---------------------------------------------------------
    else: # genre_mode == "history"
        full_instruction = f"""
    [역할]
    당신은 역사적 사건을 생생하게 재현하는 '영화 컨셉 아티스트'이자 '다큐멘터리 감독'입니다.
    ...
    2. **색감(Colors):** **색감을 풍부하게 살리십시오.** 현대적인 쨍함이 아니라, 그 시대의 명화나 필름처럼 **깊이 있고 진한 색조(Rich & Deep Tones)**를 사용하여 시대적 분위기를 연출하십시오.
    3. **분위기(Mood):** 가볍지 않고 진중하며, 마치 그 시대에 있는 듯한 현장감 넘치는 분위기를 만드십시오.
    4. **디테일(Detail):** 인물의 복식, 건축 양식, 소품 등의 역사적 고증에 신경 써서 사실감을 극대화하십시오.
    5. **텍스트(Text):** 대본의 핵심이 되는 **한글 키워드 1~2개**를 장면에 자연스럽게 녹여내십시오. (예: 오래된 현판, 깃발, 펼쳐진 문서, 비석 등에 쓰인 글씨처럼 연출)
