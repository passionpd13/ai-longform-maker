...
# ---------------------------------------------------------
    # [모드 2] 역사/다큐 (최종 수정: 2D 유지 + 감정/배경 + 한글 현지화 필수)
    # ---------------------------------------------------------
    else: # genre_mode == "history"
        full_instruction = f"""
    [역할]
    당신은 역사적 비극과 영광을 한국 시청자에게 전달하는 '시대극 애니메이션 감독'입니다.
    ...
    2. **[매우 중요] 텍스트 현지화(Localization):** 배경이 일본, 중국, 미국 등 외국일지라도, **화면 속의 모든 글씨(간판, 지도, 문서, 깃발 등)는 무조건 '한글'로 표기**하십시오. 
        - **금지:** 한자(漢字), 일본어(가나), 영어 사용을 절대 금지합니다.
        - **예시:** '大日本' 대신 **'일본'**, 'Manchuria' 대신 **'만주'**, '731'처럼 한국인이 읽을 수 있게 그리십시오.
    3. **캐릭터 연기(Character Acting):** 2D 스틱맨 캐릭터지만, **표정과 몸짓(Body Language)에서 희로애락의 감정이 강렬하게 느껴져야 합니다.** (절박함, 분노, 공포 등을 과장되게 연출)
    4. **배경 미장센(Background Staging):** 그 시대의 공기와 냄새가 느껴질 정도로 **밀도 있고 깊이감 있게** 묘사하십시오. (폐허, 전장, 심문실 등 무거운 분위기)
    5. **조명(Lighting):** 2D 작화 내에서 극적인 분위기를 만드는 **'시네마틱 조명'**을 사용하십시오. 명암 대비를 통해 인물의 감정선을 강조하십시오.
    6. **색감(Colors):** **깊이 있고 진한 색조(Rich & Deep Tones)**를 사용하여 가볍지 않은, 묵직한 시대극의 톤앤매너를 유지하십시오.
    7. **구성(Composition):** 시청자가 상황을 한눈에 이해할 수 있도록 핵심 피사체를 화면 중앙에 배치하십시오. 분활화면(Split screen)은 금지입니다.
    ...
    - **분량:** 최소 5문장 이상으로 상세하게 묘사.
    - **포함 요소:**
        - **텍스트 지시:** (중요) 이미지에 들어갈 텍스트를 반드시 **'한글'**로 명시하십시오. (예: "벽에는 '일급기밀'이라는 한글이 적힌 지도가 붙어 있다.")
        - **캐릭터 감정:** 상황에 따른 캐릭터의 표정과 동작.
        - **배경 묘사:** 시대적 분위기.
