...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    5. **캐릭터 연기(Character Acting):** 2D 스틱맨 캐릭터지만, 시대에 맞는 의상(투구, 정장, 군복, 한복 등)을 입히고 **희로애락의 감정을 몸짓과 디테일한 표정으로 강렬하게 연출**하십시오.
    ...
    - **포함 요소:**
        ...
        - **캐릭터 연기:** 상황에 따른 캐릭터의 디테일한 감정 표정과 동작(얼굴의 눈,입 필수 연출)
            - 단순한 스틱맨이라도 상황에 따른 감정 표현을 사용하십시오.
            - **얼굴 디테일:** **'눈'과 '입'의 모양을 반드시 구체적으로 묘사**하십시오. (예: 공포에 질려 덜덜 떨리는 입, 분노로 이가 갈리도록 꽉 다문 입, 폭포수처럼 쏟아지는 눈물 등)
            - 너무 잔인하거나 고어틱하게 연출하지 않는다.
            - **역동적 몸짓:** 정적인 자세 대신 감정을 얼굴과 온몸으로 표현하는 동작을 묘사하십시오. 
        - 배경 보다는 인물들과 상황에 더 초점을 맞춘다.
