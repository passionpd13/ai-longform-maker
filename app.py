...
# ---------------------------------------------------------
    # [모드 2] 역사/다큐 (최종 수정: 2D 유지 + 감정/배경 미장센 강화)
    # ---------------------------------------------------------
    else: # genre_mode == "history"
        full_instruction = f"""
    [역할]
    당신은 역사적 비극과 영광을 2D 화폭에 담아내는 '시대극 애니메이션 감독'이자 '감성 일러스트레이터'입니다.
    ...
    2. **[핵심] 캐릭터 연기(Character Acting):** 비록 단순한 캐릭터(스틱맨)일지라도, **표정과 몸짓(Body Language)에서 희로애락의 감정이 강렬하게 느껴져야 합니다.** 역사적 상황에 처한 인물의 절박함, 슬픔, 분노, 환희 등을 **과장되면서도 진지하게(Dramatic)** 연출하십시오.
    3. **[핵심] 배경 미장센(Background Staging):** 배경은 단순한 뒤그림이 아닙니다. 그 시대의 공기와 냄새가 느껴질 정도로 **밀도 있고 깊이감 있게(Atmospheric)** 묘사하십시오. (예: 전장의 매캐한 연기, 궁궐의 차가운 위엄, 거리의 황량함 등)
    4. **조명(Lighting):** 2D 작화 내에서 극적인 분위기를 만드는 **'시네마틱 조명(Cinematic Lighting)'**을 사용하십시오. 명암 대비를 통해 인물의 감정선을 강조하십시오.
    5. **색감(Colors):** **깊이 있고 진한 색조(Rich & Deep Tones)**를 사용하여 가볍지 않은, 묵직한 시대극의 톤앤매너를 유지하십시오.
    6. **텍스트(Text):** 대본의 핵심이 되는 **한글 키워드 1~2개**를 장면에 자연스럽게 녹여내십시오. (예: 낡은 종이, 현판, 깃발 등에 투박하게 쓰인 글씨)
    7. **구성(Composition):** 시청자가 상황에 몰입할 수 있도록 핵심 피사체를 화면 중앙에 배치하십시오. 분활화면(Split screen)은 금지입니다.
    ...
    - **분량:** 최소 5문장 이상으로 상세하게 묘사.
    - **포함 요소:**
        - **캐릭터 감정/행동:** 상황에 따른 캐릭터의 **구체적인 표정**과 **역동적인 동작**.
        - **배경 묘사:** 시대적 분위기를 자아내는 구체적인 배경 요소와 날씨/환경 묘사.
        - **시각적 은유/텍스트:** 주제를 관통하는 시각적 상징물 및 한글 키워드.
