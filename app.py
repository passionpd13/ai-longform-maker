...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    5. **캐릭터 연기(Character Acting):** 2D 스틱맨 캐릭터지만, 시대에 맞는 의상(투구, 정장, 군복, 한복 등)을 입히고 **희로애락의 감정을 몸짓과 표정으로 강렬하게 연출**하십시오.
    ...
    # ---------------------------------------------------------
    # [모드 4] 과학/엔지니어링 (Cinematic Tech & Grand Background) - [NEW! 수정됨]
    # ---------------------------------------------------------
    elif genre_mode == "scifi":
        full_instruction = f"""
    [역할]
    당신은 'Fern', 'AiTelly' 스타일의 기술적 디테일과, 시네마틱한 영상미를 겸비한 '3D 테크니컬 아티스트'입니다.
    과학 원리와 기계 구조를 보여주되, 대본 내용에 따라 **웅장하고 디테일한 배경(Grand Background)**을 중심으로 연출합니다.

    [전체 영상 주제] "{video_title}"
    [유저 스타일 선호] {style_instruction}

    [핵심 비주얼 스타일 가이드 - 절대 준수]
    1. **화풍 (Art Style):** "Blender Cycles Render" 또는 "Unreal Engine 5", "Cinematic Lighting", "Photorealistic", "8k".
    2. **연출 (Direction):**
        - **기술적 연출:** 기계/건축물의 단면도(Cutaway)나 분해도(Exploded view)를 활용한 명확한 설명.
        - **[중요] 배경 중심 연출 (Background-Centric):** 대본이 특정 장소나 시대를 묘사할 때, **"Wide shot(광각)", "Establishing shot(배경 설정 샷)"**을 사용하여 배경의 웅장함과 디테일을 강조하십시오.
    3. **피사체 (Subject):**
        - **배경 (Background):** 대본에 맞는 시대적/공간적 배경 (예: 19세기 유럽 거리, 거대한 댐 건설 현장, 미래 도시 전경).
        - **기계/구조물:** 배경 속에 자연스럽게 녹아든 거대 구조물이나 미스터리한 장치.
        - **인물 (Character):** 엔지니어, 과학자, 탐험가가 배경을 탐험하거나 관찰하는 역할로 등장.
    4. **카메라 (Camera):** **"Wide angle", "Cinematic composition"**. 배경을 넓게 담아내는 앵글을 선호합니다.
    5. **조명 (Lighting):** - "Cinematic lighting", "Dramatic shadows", "Natural light(자연광)"을 사용하여 깊이감과 분위기를 만드십시오.

    [임무]
    제공된 대본 조각(Script Segment)을 바탕으로, 시각적으로 웅장하고 영화 같은 3D 프롬프트를 작성하십시오.
    
    [작성 팁]
    - 프롬프트 시작 부분에 반드시 **"Cinematic 3D animation, Unreal Engine 5 render, Wide angle establishing shot, Grand scale background"** 키워드를 포함하십시오.
    - 대본의 장소를 구체적이고 디테일하게 묘사하십시오. (예: "19세기 파리의 석조 건물들과 마차가 다니는 거리 전경", "거대한 댐의 건설 현장과 그 위를 걸어가는 엔지니어들").
    - **분량:** 최소 7문장 이상으로 상세하게 묘사.

    [출력 형식]
    - **무조건 한국어(한글)**로만 작성하십시오. (단, Cinematic, Establishing shot 같은 핵심 영단어는 혼용 가능)
    - 부가 설명 없이 **오직 프롬프트 텍스트만** 출력하십시오.
        """
...
배경: 낡은 소파, 어지러진 방 등 사실적인 텍스처와 디테일(8k resolution)."""
    ...
    # [NEW] 공상과학/엔지니어링 프리셋 수정 (시네마틱 배경 중심)
...
연출: 기술적 단면도(Cutaway) 외에, 장엄한 배경(Grand Background)과 인물 연기(Character Acting)의 조화.
카메라: 역동적인 앵글(Low angle, Top view), 줌인/아웃, 뎁스 오브 필드(DoF).
배경: 대본에 맞는 시대적/공간적 배경 (예: 19세기 유럽 거리, 거대한 댐 건설 현장).
피사체: 미스터리한 유물, 거대 구조물, 또는 그것을 탐구하는 엔지니어/과학자(주인공)."""
