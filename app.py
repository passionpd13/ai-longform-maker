...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean", character_desc=""):
    ...
    **얼굴 표정이 단순한 스틱맨이므로, '몸짓(Body Language)'으로 감정을 표현하는 데 집중하십시오.**
    ...
    3. **[★★★핵심 - 캐릭터 표정 및 행동 연출 강화★★★]:**
        - **가장 중요:** 단순한 졸라맨이지만, **얼굴 표정과 행동을 과장하여(Exaggerated) 감정을 극적으로 전달**해야 합니다.
        - **표정:** 점 눈과 선 입을 기본으로 하되, 상황에 따라 **눈을 엄청 크게 그리거나, 입을 찢어지게 벌리거나, 얼굴 밖으로 튀어나오는 눈물/콧물** 등을 과감하게 표현하여 감정이 '잘 보이게' 하십시오.
        - **행동(Body Language):** 정적인 자세를 피하십시오. **온몸을 사용한 역동적인 포즈** (예: 땅을 치며 통곡, 하늘 높이 점프하며 환호, 너무 놀라 뒤로 자빠짐)로 상황을 설명하십시오.
