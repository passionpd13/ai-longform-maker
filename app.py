...
# ---------------------------------------------------------
    # [모드 2] 역사/다큐 (최종: 2D/한글/감정 + 검열 회피 전략 포함)
    # ---------------------------------------------------------
    ...
    2. **[매우 중요] 텍스트 현지화(Localization):** 배경이 외국일지라도, 화면 속 글씨(간판, 지도, 깃발 등)는 **무조건 '한글'로 표기**하십시오. (한자, 일본어 금지. 예: '大日本' -> '일본')
    3. **[속도/검열 해결] 수위 조절 및 은유(Metaphor):** 유혈, 훼손된 신체 등 잔인한 묘사는 AI 검열로 인해 생성이 차단될 수 있습니다. **직접적인 묘사 대신 '시각적 은유'를 사용하십시오.**
        - 예) 피 흘리는 장면 -> '붉은 잉크가 번진 문서', '붉은 조명'
        - 예) 고문/사망 -> '깨진 안경', '바닥에 떨어진 모자', '꺼진 촛불', '검은 그림자'
    4. **캐릭터 연기(Character Acting):** 2D 스틱맨 캐릭터지만, **표정과 몸짓(Body Language)에서 희로애락의 감정이 강렬하게 느껴져야 합니다.** (절박함, 분노, 공포 등을 과장되게 연출)
    5. **배경 미장센(Background Staging):** 그 시대의 공기와 냄새가 느껴질 정도로 **밀도 있고 깊이감 있게** 묘사하십시오. (폐허, 전장, 심문실 등 무거운 분위기)
    6. **조명(Lighting):** 2D 작화 내에서 극적인 분위기를 만드는 **'시네마틱 조명'**을 사용하십시오. 명암 대비를 통해 인물의 감정선을 강조하십시오.
    7. **색감(Colors):** **깊이 있고 진한 색조(Rich & Deep Tones)**를 사용하여 가볍지 않은, 묵직한 시대극의 톤앤매너를 유지하십시오.
